...
@section('content')
  <div class="person-info border-b border-gray-800">
    <div class="container mx-auto px-4 md:px-20 py-16 flex flex-col md:flex-row">
      <img src="https://image.tmdb.org/t/p/w500/{{ $person['profile_path'] }}" alt="profile-path" class="w-96">
      <div class="ml-2 md:ml-24 mt-8 md:mt-0">
        ...
        <div class="flex flex-wrap items-center text-gray-400 text-sm mt-2">
          ...
          <span>{{ \Carbon\Carbon::parse($person['birthday'])->format('M d, Y') }}</span>
          <span class="mx-2">|</span>
          <span>
    ...
    <div class="container mx-auto px-4 md:px-20 py-16">
      <h2 class="font-semibold text-4xl">Movie Credits</h2>
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
        @foreach ($movies as $movie)
          <x-movie-card :url="'movie'" :movie="$movie" :genres="$genres" />
        @endforeach
        @foreach ($tvShows as $tvShow)
          <x-movie-card :url="'tv-show'" :movie="$tvShow" :genres="$genres" />
        @endforeach
      </div>
    </div>
  </div>
